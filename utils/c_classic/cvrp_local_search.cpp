...
extern "C" {
	int* cvrp_local_search(
		...
		// Count actual number of routes in the input tour and ensure nbVehicles is large enough
		int nRoutesInTour = 0;
		bool inRoute = false;
		for (int i = 0; i < tour_length; i++) {
			if (tour[i] > 0) inRoute = true;
			else if (inRoute) { nRoutesInTour++; inRoute = false; }
		}
		if (inRoute) nRoutesInTour++;
		if (nRoutesInTour > params.nbVehicles)
			params.nbVehicles = nRoutesInTour;
